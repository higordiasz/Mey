...
module.exports = {
    ...
    execute(msg, args, client) {
        ...
        if (!db.get(`rpg.characters.players.${user.id}`)) return msg.reply('Este usuario não possui um profile ainda.').then(msg => {

        })
        let userinfo = db.get(`rpg.characters.players.${user.id}`)
        let val = {
